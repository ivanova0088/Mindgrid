/* MG_Metro — محرك نمط المترو (Fix M1: نسبة مئوية + viewBox) */
(function(){
  'use strict';
  ...
  function drawLine(svg, pts, color){
    // نستخدم وحدات viewBox (0..100) مباشرة
    const d = pts.map((p,i)=> (i?'L':'M') + p.x + ' ' + p.y).join(' ');
    const path = document.createElementNS('http://www.w3.org/2000/svg','path');
    path.setAttribute('d', d);
    path.setAttribute('stroke', color);
    path.setAttribute('stroke-width', '1.6'); // مناسب لوحدة viewBox
  ...
  function placeInput(wrap, cell, idx){
    ...
    // موضع بالنسبة المئوية مباشرة (من 0 إلى 100)
    const left = cell.x + '%';
    const top  = cell.y + '%';
  ...
  function start(opts){
    ...
    // تأكد من viewBox حتى يعمل القياس النسبي على جميع مقاسات الشاشة
